...
public class TokenProvider implements Serializable {
    ...
    private Claims getAllClaimsFromToken(String token) {
        return Jwts.parserBuilder().setSigningKey(Base64.getEncoder().encode(jwtConfig.getHmacSecret().getBytes()))
    ...
    public UsernamePasswordAuthenticationToken getAuthenticationToken(final String token, final UserDetails userDetails) {
        final var jwtParser = Jwts.parserBuilder().setSigningKey(Base64.getEncoder().encode(jwtConfig.getHmacSecret().getBytes()));
